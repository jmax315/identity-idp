...
RSpec.feature 'Sign in with multiple vectors of trust', allowed_extra_analytics: [:*] do
  ...
  context 'proofing or no proofing is acceptable (IALMAX)' do
    scenario 'identity proofing is not required if the user is not proofed'

    scenario 'attributes are shared if the user is proofed'

    scenario 'identity proofing is not required if proofed user resets password'
  end
end
