      ...
      context 'on mobile platform', allow_browser_log: true do
        ...
        context 'selfie with error is uploaded' do
          context 'IPP enabled' do
            before do
              # TODO: Verify which of top two is correct
            ...
            it 'shows the correct error message for the given error' do
              ...
              perform_in_browser(:mobile) do
                ...
                # falsely passing - no subheading, just the same text in body message, need to check type of element
                review_issues_subheading = strip_tags(t('errors.doc_auth.rate_limited_subheading'))
                expect(page).to have_content(review_issues_subheading)
                ...
                # currently failing
                # review_issues_rate_limit_warning = strip_tags(t('idv.warning.attempts_html.other'))
                # expect(page).to have_content(review_issues_rate_limit_warning)

                click_try_again

                # currently failing
                # resubmit_page_body_copy = strip_tags(t('doc_auth.errors.dpi.top_msg_plural'))
                # expect(page).to have_content(resubmit_page_body_copy)

                resubmit_page_inline_error_messages = strip_tags(t('doc_auth.errors.general.fallback_field_level'))
                expect(page).to have_content(resubmit_page_inline_error_messages).twice

                resubmit_page_inline_selfie_error_message = strip_tags(t('doc_auth.errors.general.selfie_failure'))
                ...
                review_issues_body_message = strip_tags(t('doc_auth.errors.general.multiple_front_id_failures'))
                expect(page).to have_content(review_issues_body_message)
                ...
                review_issues_body_message = strip_tags(t('doc_auth.errors.general.multiple_back_id_failures'))
                expect(page).to have_content(review_issues_body_message)
                ...
                resubmit_page_body_copy = strip_tags(t('doc_auth_errors.dpi.top_msg'))
                expect(page).to have_content(resubmit_page_body_copy)
